...
const Feed = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      const postList = await Promise.all(postSnapshot.docs.map(async (doc) => {
        ...
        // Buscar as informações do autor na coleção 'users'
        const userDoc = await getDoc(doc(db, 'users', postData.userId)); // Supondo que 'userId' está no post
  ...
  const PostItem = ({ texto, img, autor, setModalVisible }) => { 
    return (
      <View style={{ borderColor: "#dedede", borderBottomWidth: 1, paddingVertical: 5, backgroundColor: '#FFF' }}>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <Image 
            source={{ uri: autor?.perfil || 'https://s1.static.brasilescola.uol.com.br/be/2022/10/meme-mo-paz.jpg' }} 
            style={{ width: 40, height: 40, borderRadius: 25, marginRight: 10, margin: 5 }} 
          />
          <Text style={{ fontWeight: 'bold' }}>{autor?.name || 'Unknown'}</Text> 
        </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <PostItem
            texto={item.texto}
            img={item.img}
            autor={item.autor} // Passa as informações do autor para o PostItem
